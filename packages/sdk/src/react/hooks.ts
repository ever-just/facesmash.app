...
/** Hook for face login flow (API mode — sends descriptor to server) */
export function useFaceLogin() {
  ...
  const login = useCallback(
    async (descriptor: number[] | Float32Array): Promise<LoginResult> => {
      if (!isReady) {
      ...
      const loginResult = await client.login(descriptor);
...
/** Hook for face registration flow (API mode) */
export function useFaceRegister() {
  ...
  const register = useCallback(
    async (params: {
      ...
      const regResult = await client.register(params);
      setResult(regResult);
