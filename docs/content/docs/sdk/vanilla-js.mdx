...
3. The descriptor is sent to the Hono API for server-side pgvector matching
4. For each profile:
...
Direct access to the underlying API client for custom queries against the Hono API.
...
```javascript
// The Hono API handles all database operations server-side.
// Use the SDK's high-level methods for all face operations:
const loginResult = await client.login(images);
const registerResult = await client.register(name, images, email);

// Session management is handled via httpOnly cookies automatically.
// Check the current authenticated user:
const me = await fetch('https://api.facesmash.app/api/auth/me', {
  credentials: 'include',
}).then(r => r.json());
```
