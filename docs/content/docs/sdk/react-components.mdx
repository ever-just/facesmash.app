...
|---|---|---|
| `client` | `FaceSmashClient` | The initialized SDK client — access `client.login()`, `client.register()`, `client.analyzeFace()`, `client.on()`, `.config`, `.isReady` |
...
<Callout type="warn">
  **Do not nest multiple providers.** Each `FaceSmashProvider` creates its own `FaceSmashClient` instance. Use a single provider at the top of your component tree.
</Callout>
...
Drop-in login component. Renders a mirrored webcam feed, waits 2 seconds for the user to position their face, then auto-captures multiple frames and authenticates via the Hono API's pgvector matching.
...
Drop-in registration component. Renders a mirrored webcam feed, captures face images, checks for duplicate faces via server-side pgvector matching, and creates a new user profile in PostgreSQL.
...
| Field | Type | Description |
|---|---|---|
| `client` | `FaceSmashClient` | The SDK client — call `.login()`, `.register()`, `.analyzeFace()`, `.on()`, access `.config`, `.isReady` |
